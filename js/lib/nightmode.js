...
class Nightmode 
{
	...
	Initialize() 
	{
		...
		$(document).on('keyup', '#auto-night-mode-time-range-from, #auto-night-mode-time-range-to', function () 
		{
			...
			const valueIsValid = moment(value, 'HH:mm', true).isValid();
	...
	CheckNightMode() 
	{
		...
		const start = moment(this.Grocy.UserSettings.auto_night_mode_time_range_from, 'HH:mm', true);
		const end = moment(this.Grocy.UserSettings.auto_night_mode_time_range_to, 'HH:mm', true);
		const now = moment();
