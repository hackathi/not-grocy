...
class datetimepicker 
{
	...
	SetValue(value, triggerEvents = true) 
	{
		...
		const shortcutValue = this.$('#' + this.basename + '-shortcut').data(this.basename + '-shortcut-value');
	...
	keyupHandler(_this, e) 
	{
		...
		const now = new Date();
		const centuryStart = Number.parseInt(now.getFullYear().toString().substring(0, 2) + '00');
		const centuryEnd = Number.parseInt(now.getFullYear().toString().substring(0, 2) + '99');
		const format = inputElement.data('format');
		const nextInputElement = this.$(inputElement.data('next-input-selector')); // If input is empty and any arrow key is pressed, set date to today
		...
		{
			const date = moment(new Date().getFullYear().toString() + value);
		...
		{
			const endOfMonth = moment(value.substring(0, 4) + '-' + value.substring(4, 6) + '-01').endOf('month');
		...
		else 
		{
			...
			const earlierThanLimit = inputElement.data('earlier-than-limit');
	...
	inputHandler(_this, e) 
	{
		...
		EmptyElementWhenMatches(this.$('#' + this.basename + '-timeago'), this.Grocy.translate('timeago_nan')); // TODO: scoping
	...
	stateTrigger() 
	{
		const linputElement = this.GetInputElement();
		linputElement.trigger('input').trigger('change').trigger('keypress').trigger('keyup');
	}
	...
	{
		{
			...
			if (_this.checked) 
			{
				const value = this.$('#' + this.basename + '-shortcut').data(this.basename + '-shortcut-value');
				this.SetValue(value);
