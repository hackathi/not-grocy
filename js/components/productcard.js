...
class productcard 
{
	constructor(Grocy, scopeSelector = null) 
	{
		...
		{
			self.$('.expandable-text').find("a[data-toggle='collapse']").text(self.Grocy.translate('Show less'));
		});
		this.$('#productcard-product-description').on('hidden.bs.collapse', function () 
		{
			self.$('.expandable-text').find("a[data-toggle='collapse']").text(self.Grocy.translate('Show more'));
		}); // preload some views.
	...
	{
		const self = this;
		this.Grocy.Api.Get('stock/products/' + productId, function (productDetails) 
		{
			const stockAmount = productDetails.stock_amount || '0';
			const stockValue = productDetails.stock_value || '0';
			const stockAmountOpened = productDetails.stock_amount_opened || '0';
			...
			self.$('#productcard-product-spoil-rate').text((parseFloat(productDetails.spoil_rate_percent) / 100).toLocaleString(undefined, {
				style: 'percent'
			}));
		...
		{
			this.Grocy.Api.Get('stock/products/' + productId + '/price-history', function (priceHistoryDataPoints) 
			{
				if (priceHistoryDataPoints.length > 0) 
				{
					...
					const datasets = {};
					const chart = self.PriceHistoryChart.data;

					for (const dataPoint of priceHistoryDataPoints) 
					{
