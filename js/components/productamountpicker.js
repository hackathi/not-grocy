...
class productamountpicker 
{
	...
	{
		const conversionsForProduct = this.Grocy.QuantityUnitConversionsResolved.filter(elem => elem.product_id == productId);

		if (!this.AllowAnyQuEnabled) 
		{
			const qu = this.Grocy.QuantityUnits.find(elem => elem.id == destinationQuId);
			...
			for (const conversion of conversionsForProduct) 
			{
				let factor = parseFloat(conversion.factor);
		...
		{
			const convertedAmount = this.display_amount.val() * this.$('#qu_id option:selected').attr('data-qu-factor');
	...
	AllowAnyQu(keepInitialQu = false) 
	{
		...
		for (const qu of this.Grocy.QuantityUnits) 
		{
	...
	{
		const selectedQuName = this.$('#qu_id option:selected').text();
		const quFactor = this.$('#qu_id option:selected').attr('data-qu-factor');
		const amount = this.display_amount.val();
		const destinationAmount = amount / quFactor;
		const destinationQuName = this.Grocy.translaten(destinationAmount, this.qu_id.attr('data-destination-qu-name'), this.qu_id.attr('data-destination-qu-name-plural'));
		const conversionInfo = this.$('#qu-conversion-info');

		if (this.qu_id.attr('data-destination-qu-name') == selectedQuName || this.AllowAnyQuEnabled || amount.toString().isEmpty() || selectedQuName.toString().isEmpty()) 
		{
		...
		else 
		{
			...
			conversionInfo.text(this.Grocy.translate('This equals %1$s %2$s', destinationAmount.toLocaleString({
				minimumFractionDigits: 0,
				maximumFractionDigits: this.Grocy.UserSettings.stock_decimal_places_amounts
			}), destinationQuName));
		}

		$('#amount').val(destinationAmount.toFixed(this.Grocy.UserSettings.stock_decimal_places_amounts).replace(/0*$/g, ''));
	}
