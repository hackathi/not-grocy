...
class productpicker extends BasePicker 
{
	...
	{
		let doFocus = false;
		let prefillProduct = this.Grocy.GetUriParam('product-name');
		const prefillProduct2 = this.picker.parent().data('prefill-by-name').toString();
		...
		{
			let possibleOptionElement = this.$('#product_id option[data-additional-searchdata*="' + prefillProduct + '"]').first();
		...
		let prefillProductId = this.Grocy.GetUriParam('product');
		const prefillProductId2 = this.picker.parent().data('prefill-by-id').toString();
	...
	{
		const element = this.$('#custom-productpicker-error');
		element.text(text);
	...
	onBlurHandler(_this, e) 
	{
		...
		const input = this.$('#product_id_text_input').val().toString();
		let possibleOptionElement = []; // did we enter a grocycode?

		if (input.startsWith('grcy')) 
		{
			const gc = input.split(':');
		...
		else 
		{
			...
			const optionElement = this.$('#product_id option:contains("' + input + '")').first();

			if (input.length > 0 && optionElement.length === 0 && this.Grocy.GetUriParam('flow') === undefined && this.picker.parent().data('disallow-all-product-workflows').toString() === 'false') 
			{
				let addProductWorkflowsAdditionalCssClasses = '';
				...
				const buttons = {
					...
					addnewproduct: {
						...
						{
							self.PopupOpen = false;
							window.location.href = self.Grocy.FormatUrl('/product/new?flow=InplaceNewProductWithName' + '&name=' + encodeURIComponent(input) + '&returnto=' + encodeURIComponent(self.Grocy.CurrentUrlRelative + '?flow=InplaceNewProductWithName' + '&' + self.embedded) + '&' + self.embedded);
						}
					},
					addbarcode: {
						...
						{
							self.PopupOpen = false;
							window.location.href = self.Grocy.FormatUrl(self.Grocy.CurrentUrlRelative + '?flow=InplaceAddBarcodeToExistingProduct' + '&barcode=' + encodeURIComponent(input));
						}
					},
					addnewproductwithbarcode: {
						...
						{
							self.PopupOpen = false;
							window.location.href = self.Grocy.FormatUrl('/product/new' + '?flow=InplaceNewProductWithBarcode' + '&barcode=' + encodeURIComponent(input) + '&returnto=' + encodeURIComponent(self.Grocy.CurrentUrlRelative + '?flow=InplaceAddBarcodeToExistingProduct' + '&barcode=' + input + '&' + self.embedded) + '&' + self.embedded);
						}
					}
				};
	...
	onBarcodeScannedHandler(e, barcode, target) 
	{
		...
		this.input_element.focusout().focus().blur();
		this.input_element.val(barcode);
		setTimeout(function () 
		{
			self.input_element.focusout().focus().blur();
		}, 200);
	}
