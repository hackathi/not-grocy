...
function shoppinglistformView(Grocy, scope = null) 
{
	...
	$scope('#save-shopping-list-button').on('click', function (e) 
	{
		...
		const jsonData = $scope('#shopping-list-form').serializeJSON();
		Grocy.FrontendHelpers.BeginUiBusy('shopping-list-form');

		if (Grocy.EditMode === 'create') 
		{
			Grocy.Api.Post('objects/shopping_lists', jsonData, function (result) 
			{
		...
		else 
		{
			userfields.Save(function () 
			{
				Grocy.Api.Put('objects/shopping_lists/' + Grocy.EditObjectId, jsonData, function (result) 
				{
