...
function quantityunitformView(Grocy, scope = null) 
{
	...
	const userfields = Grocy.Use('userfieldsform'); // preload some views.

	Grocy.PreloadView('quantityunitconversionform');
	$scope('.save-quantityunit-button').on('click', function (e) 
	{
		e.preventDefault();
		const jsonData = $scope('#quantityunit-form').serializeJSON();
		Grocy.FrontendHelpers.BeginUiBusy('quantityunit-form');
		let redirectDestination = U('/quantityunits');
		...
		{
			Grocy.Api.Post('objects/quantity_units', jsonData, function (result) 
			{
		...
		{
			Grocy.Api.Put('objects/quantity_units/' + Grocy.EditObjectId, jsonData, function (result) 
			{
				userfields.Save(function () 
				{
					if (Grocy.GetUriParam('embedded') !== undefined) 
	...
	const quConversionsTable = $scope('#qu-conversions-table').DataTable({
		order: [[1, 'asc']],
		columnDefs: [{
			orderable: false,
			targets: 0
	...
	$scope('#name').focus();
	Grocy.FrontendHelpers.ValidateForm('quantityunit-form');
	Grocy.FrontendHelpers.MakeDeleteConfirmBox('Are you sure to remove this conversion?', '.qu-conversion-delete-button', 'data-qu-conversion-id', 'data-qu-conversion-id', 'objects/quantity_unit_conversions/', () => window.location.reload()); // TODO: LoadSubView
