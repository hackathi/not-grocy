...
function transferView(Grocy, scope = null) 
{
	...
	$scope('#save-transfer-button').on('click', function (e) 
	{
		...
		const jsonForm = $scope('#transfer-form').serializeJSON();
		Grocy.FrontendHelpers.BeginUiBusy('transfer-form');
		const apiUrl = 'stock/products/' + jsonForm.product_id + '/transfer';
		const jsonData = {};
		...
		Grocy.Api.Get('stock/products/' + jsonForm.product_id, function (productDetails) 
		{
			Grocy.Api.Post(apiUrl, jsonData, function (result) 
			{
				...
				amount += ' ' + __n(jsonForm.amount, productDetails.quantity_unit_stock.name, productDetails.quantity_unit_stock.name_plural);
				const successMessage = __t('Transfered %1$s of %2$s from %3$s to %4$s', amount, productDetails.product.name, $scope('option:selected', '#location_id_from').text(), $scope('option:selected', '#location_id_to').text()) + '<br><a class="btn btn-secondary btn-sm mt-2" href="#" onclick="Grocy.UndoStockTransaction(\'' + bookingResponse[0].transaction_id + '\')"><i class="fas fa-undo"></i> ' + __t('Undo') + '</a>';
	...
	productpicker.GetPicker().on('change', function (e) 
	{
		...
		if (productId) 
		{
			...
			Grocy.Api.Get('stock/products/' + productId, function (productDetails) 
			{
				...
				$scope('#location_id_from').find('option').remove().end().append('<option></option>');
				Grocy.Api.Get('stock/products/' + productId + '/locations', function (stockLocations) 
				{
					let setDefault = 0;
	...
	$scope('#location_id_from').on('change', function (e) 
	{
		...
		if (locationId) 
		{
			Grocy.Api.Get('stock/products/' + productpicker.GetValue() + '/entries', function (stockEntries) 
			{
