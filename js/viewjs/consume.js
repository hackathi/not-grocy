...
function consumeView(Grocy, scope = null) 
{
	...
	$scope('#save-consume-button').on('click', function (e) 
	{
		...
		const jsonForm = $scope('#consume-form').serializeJSON();
		Grocy.FrontendHelpers.BeginUiBusy('consume-form');
		const apiUrl = 'stock/products/' + jsonForm.product_id + '/consume';
		const jsonData = {};
		...
		{
			Grocy.Api.Post(apiUrl, jsonData, function (result) 
			{
	...
	$scope('#save-mark-as-open-button').on('click', function (e) 
	{
		...
		const jsonForm = $scope('#consume-form').serializeJSON();
		Grocy.FrontendHelpers.BeginUiBusy('consume-form');
		const apiUrl = 'stock/products/' + jsonForm.product_id + '/open';
		const jsonData = {};
		jsonData.amount = jsonForm.amount;
		jsonData.allow_subproduct_substitution = true;
		...
		Grocy.Api.Get('stock/products/' + jsonForm.product_id, function (productDetails) 
		{
			Grocy.Api.Post(apiUrl, jsonData, function (result) 
			{
				...
				}) + ' ' + __n(jsonForm.amount, productDetails.quantity_unit_stock.name, productDetails.quantity_unit_stock.name_plural), productDetails.product.name) + '<br><a class="btn btn-secondary btn-sm mt-2" href="#" onclick="Grocy.UndoStockTransaction(\'' + result[0].transaction_id + '\')"><i class="fas fa-undo"></i> ' + __t('Undo') + '</a>');
	...
	{
		const locationId = $scope(e.target).val();
		sumValue = 0;
		let stockId = null;
		$scope('#specific_stock_entry').find('option').remove().end().append('<option></option>');
		...
		else 
		{
			...
			{
				const gc = $scope('#product_id').attr('barcode').split(':');
		...
		if (locationId) 
		{
			Grocy.Api.Get('stock/products/' + productpicker.GetValue() + '/entries?include_sub_products=true', function (stockEntries) 
			{
				stockEntries.forEach(stockEntry => 
				{
					let openTxt = __t('Not opened');
				...
				Grocy.Api.Get('stock/products/' + productpicker.GetValue(), function (productDetails) 
				{
				...
				if (BoolVal(Grocy.UserSettings.scan_mode_consume_enabled) && $scope('#product_id').attr('barcode') == 'null') 
				{
	...
	productpicker.GetPicker().on('change', function (e) 
	{
		...
		if (productId) 
		{
			...
			Grocy.Api.Get('stock/products/' + productId, function (productDetails) 
			{
				...
				$scope('.input-group-productamountpicker').trigger('change');
				$scope('#location_id').find('option').remove().end().append('<option></option>');
				Grocy.Api.Get('stock/products/' + productId + '/locations?include_sub_products=true', function (stockLocations) 
				{
					let setDefault = 0;
	...
	if (Grocy.GetUriParam('embedded') !== undefined) 
	{
		const locationId = Grocy.GetUriParam('locationId');
	...
	let current_productDetails;

	function RefreshForm() 
	{
		const productDetails = current_productDetails;
