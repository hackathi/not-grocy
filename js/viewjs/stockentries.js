...
function stockentriesView(Grocy, scope = null) 
{
	...
	const productpicker = Grocy.Use('productpicker'); // preload some views.
	...
	const stockEntriesTable = $scope('#stockentries-table').DataTable({
		order: [[2, 'asc']],
		columnDefs: [{
			orderable: false,
			targets: 0
	...
	const moreSearch = function (settings, data, dataIndex) 
	{
	...
	{
		const funcIdx = $.fn.dataTable.ext.search.indexOf(moreSearch);
	...
	top.on('click', '.stock-consume-button', function (e) 
	{
		...
		const locationId = target.attr('data-location-id');
		const specificStockEntryId = target.attr('data-stock-id');
		const stockRowId = target.attr('data-stockrow-id');
		const consumeAmount = target.attr('data-consume-amount');
		const wasSpoiled = target.hasClass('stock-consume-button-spoiled');
		...
		}, function (bookingResponse) 
		{
			...
			{
				let toastMessage = __t('Removed %1$s of %2$s from stock', parseFloat(consumeAmount).toLocaleString({
					minimumFractionDigits: 0,
					maximumFractionDigits: Grocy.UserSettings.stock_decimal_places_amounts
				}) + ' ' + __n(consumeAmount, result.quantity_unit_stock.name, result.quantity_unit_stock.name_plural), result.product.name) + '<br><a class="btn btn-secondary btn-sm mt-2" href="#" onclick="Grocy.UndoStockBookingEntry(' + bookingResponse[0].id + ',' + stockRowId + ')"><i class="fas fa-undo"></i> ' + __t('Undo') + '</a>';
	...
	top.on('click', '.product-open-button', function (e) 
	{
		...
		const specificStockEntryId = button.attr('data-stock-id');
		const stockRowId = button.attr('data-stockrow-id');
	...
	top.on('click', '.stockentry-grocycode-stockentry-label-print', function (e) 
	{
		e.preventDefault();
		document.activeElement.blur();
		const stockId = $(e.currentTarget).attr('data-stock-id');
	...
	{
		Grocy.Api.Get('stock/entry/' + stockRowId, function (result) 
		{
			const stockRow = $scope('#stock-' + stockRowId + '-row'); // If the stock row not exists / is invisible (happens after consume/undo because the undone new stock row has different id), just reload the page for now
			...
			{
				const dueThreshold = moment().add(Grocy.UserSettings.stock_due_soon_days, 'days');
				const now = moment();
				const bestBeforeDate = moment(result.best_before_date);
				...
				let locationName = '';
				Grocy.Api.Get('objects/locations/' + result.location_id, function (locationResult) 
				{
				...
				let shoppingLocationName = '';
				Grocy.Api.Get('objects/shopping_locations/' + result.shopping_location_id, function (shoppingLocationResult) 
				{
