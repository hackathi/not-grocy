...
function choretrackingView(Grocy, scope = null) 
{
	...
	const chorecard = Grocy.Use('chorecard');
	...
	$scope('#save-choretracking-button').on('click', function (e) 
	{
		...
		const jsonForm = $scope('#choretracking-form').serializeJSON();
		Grocy.FrontendHelpers.BeginUiBusy('choretracking-form');
		Grocy.Api.Get('chores/' + jsonForm.chore_id, function (choreDetails) 
		{
			Grocy.Api.Post('chores/' + jsonForm.chore_id + '/execute', {
				tracked_time: datetimepicker.GetValue(),
				done_by: $scope('#user_id').val()
			}, function (result) 
			{
	...
	{
		const input = $scope('#chore_id_text_input').val().toString();
		$scope('#chore_id_text_input').val(input);
		$scope('#chore_id').data('combobox').refresh();
		const choreId = $scope(e.target).val();
