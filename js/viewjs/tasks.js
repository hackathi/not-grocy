...
function tasksView(Grocy, scope = null) 
{
	...
	const tasksTable = $scope('#tasks-table').DataTable({
		order: [[2, 'asc']],
		columnDefs: [{
			orderable: false,
			targets: 0
	...
	top.on('click', '.do-task-button', function (e) 
	{
		...
		const taskId = target.attr('data-task-id');
		const taskName = target.attr('data-task-name');
		const doneTime = moment().format('YYYY-MM-DD HH:mm:ss');
		Grocy.Api.Post('tasks/' + taskId + '/complete', {
			done_time: doneTime
		}, function () 
		{
	...
	top.on('click', '.undo-task-button', function (e) 
	{
		...
		document.activeElement.blur();
		Grocy.FrontendHelpers.BeginUiBusy();
		const taskId = $scope(e.currentTarget).attr('data-task-id');
		Grocy.Api.Post('tasks/' + taskId + '/undo', {}, function () 
		{
	...
	Grocy.FrontendHelpers.MakeDeleteConfirmBox('Are you sure to delete task "%s"?', '.delete-task-button', 'data-task-name', 'data-task-id', 'objects/tasks/', (result, objectId, objectName) => 
	{
	...
	{
		const nextXDays = $scope('#info-due-tasks').data('next-x-days');
		Grocy.Api.Get('tasks', function (result) 
		{
			let dueCount = 0;
			let overdueCount = 0;
			const now = moment();
			...
			{
				const date = moment(element.due_date);
