...
function productsView(Grocy, scope = null) 
{
	...
	const productsTable = $scope('#products-table').DataTable({
		order: [[1, 'asc']],
		columnDefs: [{
			orderable: false,
			targets: 0
	...
	Grocy.FrontendHelpers.MakeDeleteConfirmBox((objectId, objectName) => 
	{
		return __t('Are you sure to delete product "%s"?', objectName) + '<br><br>' + __t('This also removes any stock amount, the journal and all other references of this product - consider disabling it instead, if you want to keep that and just hide the product.');
	}, '.product-delete-button', 'data-product-name', 'data-product-id', 'objects/products/', '/products');
	...
	{
		const productIdToKeep = $scope('#merge-products-keep').val();
		const productIdToRemove = $scope('#merge-products-remove').val();
		Grocy.Api.Post('stock/products/' + productIdToKeep.toString() + '/merge/' + productIdToRemove.toString(), {}, function (result) 
		{
			window.location.href = U('/products');
		}, function (xhr) 
		{
