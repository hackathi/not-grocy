...
function equipmentView(Grocy, scope = null) 
{
	...
	const equipmentTable = $scope('#equipment-table').DataTable({
		order: [[1, 'asc']],
		columnDefs: [{
			orderable: false,
			targets: 0
	...
	equipmentTable.on('select', function (e, dt, type, indexes) 
	{
		if (type === 'row') 
		{
			const selectedEquipmentId = $scope(equipmentTable.row(indexes[0]).node()).data('equipment-id');
	...
	{
		Grocy.Api.Get('objects/equipment/' + id, function (equipmentItem) 
		{
			...
			{
				const pdfUrl = U('/api/files/equipmentmanuals/' + btoa(equipmentItem.instruction_manual_file_name));
	...
	Grocy.FrontendHelpers.MakeDeleteConfirmBox('Are you sure to delete equipment "%s"?', '.equipment-delete-button', 'data-equipment-name', 'data-equipment-id', 'objects/equipment/', '/equipment');
