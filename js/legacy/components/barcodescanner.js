//import { $ } from 'jquery';
import Quagga from '@ericblade/quagga2/dist/quagga'; // testitest
...
class barcodescanner 
{
	constructor(Grocy, scopeSelector = null) 
	{
		...
		this.$ = scopeSelector != null ? selector => jScope.find(selector) : $; // init component
		...
		{
			e.preventDefault();
			const inputElement = $(e.currentTarget).prev();
	...
	async CheckCapabilities() 
	{
		...
		const cameras = await Quagga.CameraAccess.enumerateVideoDevices();
		const cameraSelect = this.$('.cameraSelect-wrapper');
		...
		{
			let bc = this.$('barcodescanner-container');

			if (bc.length) 
			{
				bc = bc[0];
				const bcAspectRatio = bc.offsetWidth / bc.offsetHeight;
				const settings = track.getSettings();

				if (bcAspectRatio > settings.aspectRatio) 
				{
					let v = this.$('#barcodescanner-livestream video');

					if (v.length) 
					{
						v = v[0];
						const c = this.$('#barcodescanner-livestream canvas')[0];
