...
class BasePicker 
{
	...
	prefill() 
	{
		...
		let doFokus = false;
		const prefillByName = this.picker.parent().data('prefill-by-name').toString();

		if (typeof prefillByName !== 'undefined') 
		{
			const possibleOptionElement = $(this.basename + ' option:contains("' + prefillByName + '")').first();
		...
		const prefillById = this.picker.parent().data('prefill-by-id').toString();
