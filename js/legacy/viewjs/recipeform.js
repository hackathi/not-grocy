...
function recipeformView(Grocy, scope = null) 
{
	...
	{
		const recipeId = Grocy.EditObjectId || result.created_object_id;
		userfields.Save(() => 
		{
			if (Object.prototype.hasOwnProperty.call(jsonData, 'picture_file_name') && !Grocy.DeleteRecipePictureOnSave) 
			{
				Grocy.Api.UploadFile($scope('#recipe-picture')[0].files[0], 'recipepictures', jsonData.picture_file_name, result => 
				{
	...
	{
		e.preventDefault();
		const jsonData = $scope('#recipe-form').serializeJSON();
		Grocy.FrontendHelpers.BeginUiBusy('recipe-form');

		if ($scope('#recipe-picture')[0].files.length > 0) 
		{
			const someRandomStuff = Math.random().toString(36).substring(2, 100) + Math.random().toString(36).substring(2, 100);
		...
		{
			Grocy.Api.Post('objects/recipes', jsonData, result => saveRecipePicture(result, location, jsonData));
			return;
		}
		...
		if (Grocy.DeleteRecipePictureOnSave) 
		{
			...
			Grocy.Api.DeleteFile(Grocy.RecipePictureFileName, 'recipepictures', {}, function (result) 
			{// Nothing to do
			}, function (xhr) 
			{
				Grocy.FrontendHelpers.EndUiBusy('recipe-form');
		...
		Grocy.Api.Put('objects/recipes/' + Grocy.EditObjectId, jsonData, result => saveRecipePicture(result, location, jsonData), function (xhr) 
		{
			Grocy.FrontendHelpers.EndUiBusy('recipe-form');
			console.error(xhr);
		});
	});
	const recipesPosTables = $scope('#recipes-pos-table').DataTable({
		order: [[1, 'asc']],
		orderFixed: [[4, 'asc']],
		columnDefs: [{
			orderable: false,
	...
	const recipesIncludesTables = $scope('#recipes-includes-table').DataTable({
		order: [[1, 'asc']],
		columnDefs: [{
			orderable: false,
			targets: 0
	...
	Grocy.FrontendHelpers.MakeDeleteConfirmBox('Are you sure to delete recipe ingredient "%s"?', '.recipe-pos-delete-button', 'data-recipe-pos-name', 'data-recipe-pos-id', 'objects/recipes_pos/', () => window.postMessage(WindowMessageBag('IngredientsChanged'), Grocy.BaseUrl));
	Grocy.FrontendHelpers.MakeDeleteConfirmBox('Are you sure to remove the included recipe "%s"?', '.recipe-include-delete-button', 'data-recipe-include-name', 'data-recipe-include-id', 'objects/recipes_nesting/', () => window.postMessage(WindowMessageBag('IngredientsChanged'), Grocy.BaseUrl));
	top.on('click', '.recipe-pos-show-note-button', function (e) 
	{
		const note = $(e.currentTarget).attr('data-recipe-pos-note');
		bootbox.alert(note);
	}); // TODO: LoadSubView
	...
	top.on('click', '.recipe-pos-edit-button', function (e) 
	{
		...
		const recipePosId = $(e.currentTarget).attr('data-recipe-pos-id');
		bootbox.dialog({
	...
	{
		const id = $(e.currentTarget).attr('data-recipe-include-id');
		const recipeId = $(e.currentTarget).attr('data-recipe-included-recipe-id');
		const recipeServings = $(e.currentTarget).attr('data-recipe-included-recipe-servings');
		Grocy.Api.Put('objects/recipes/' + Grocy.EditObjectId, $scope('#recipe-form').serializeJSON(), function (result) 
		{
	...
	$scope('#save-recipe-include-button').on('click', function (e) 
	{
		...
		const nestingId = $scope('#recipe-include-form').data('recipe-nesting-id');
		const editMode = $scope('#recipe-include-form').data('edit-mode');
		const jsonData = {};
		...
		if (editMode === 'create') 
		{
			Grocy.Api.Post('objects/recipes_nestings', jsonData, function (result) 
			{
		...
		{
			Grocy.Api.Put('objects/recipes_nestings/' + nestingId, jsonData, function (result) 
			{
				window.postMessage(WindowMessageBag('IngredientsChanged'), Grocy.BaseUrl);
			}, function (xhr) 
			{
	...
	$(window).on('message', function (e) 
	{
		...
		{
			Grocy.Api.Put('objects/recipes/' + Grocy.EditObjectId, $('#recipe-form').serializeJSON(), function (result) 
			{
