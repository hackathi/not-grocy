...
function recipeposformView(Grocy, scope = null) 
{
	...
	$scope('#save-recipe-pos-button').on('click', function (e) 
	{
		...
		const jsonData = $scope('#recipe-pos-form').serializeJSON();
		...
		{
			Grocy.Api.Post('objects/recipes_pos', jsonData, function (result) 
			{
		...
		{
			Grocy.Api.Put('objects/recipes_pos/' + Grocy.EditObjectId, jsonData, function (result) 
			{
				window.parent.postMessage(WindowMessageBag('IngredientsChanged'), Grocy.BaseUrl);
				window.parent.postMessage(WindowMessageBag('CloseAllModals'), Grocy.BaseUrl);
			}, function (xhr) 
