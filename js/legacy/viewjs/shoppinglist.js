...
function shoppinglistView(Grocy, scope = null) 
{
	...
	const shoppingListTable = $scope('#shoppinglist-table').DataTable({
		order: [[1, 'asc']],
		orderFixed: [[3, 'asc']],
		columnDefs: [{
			orderable: false,
	...
	const shoppingListPrintShadowTable = $scope('#shopping-list-print-shadow-table').DataTable({
		order: [[1, 'asc']],
		orderFixed: [[2, 'asc']],
		columnDefs: [{
			visible: false,
	...
	Grocy.FrontendHelpers.MakeDeleteConfirmBox('Are you sure to delete shopping list "%s"?', '#delete-selected-shopping-list', () => $scope('#selected-shopping-list option:selected').text(), () => $scope('#selected-shopping-list').val(), 'objects/shopping_lists/', '/shoppinglist');
	top.on('click', '.shoppinglist-delete-button', function (e) 
	{
		...
		const shoppingListItemId = $(e.currentTarget).attr('data-shoppinglist-id');
		Grocy.FrontendHelpers.BeginUiBusy();
		Grocy.Api.Delete('objects/shopping_list/' + shoppingListItemId, {}, function (result) 
		{
	...
	top.on('click', '#add-products-below-min-stock-amount', function (e) 
	{
		Grocy.Api.Post('stock/shoppinglist/add-missing-products', {
			list_id: $scope('#selected-shopping-list').val()
		}, function (result) 
		{
	...
	{
		Grocy.Api.Post('stock/shoppinglist/add-overdue-products', {
			list_id: $('#selected-shopping-list').val()
		}, function (result) 
		{
			Grocy.Api.Post('stock/shoppinglist/add-expired-products', {
				list_id: $('#selected-shopping-list').val()
			}, function (result) 
			{
	...
	Grocy.FrontendHelpers.MakeYesNoBox(() => __t('Are you sure to empty shopping list "%s"?', $scope('#selected-shopping-list option:selected').text()), '#clear-shopping-list', result => 
	{
		if (result === true) 
		{
			Grocy.FrontendHelpers.BeginUiBusy();
	...
	top.on('click', '.shopping-list-stock-add-workflow-list-item-button', function (e) 
	{
		...
		document.activeElement.blur();
		const href = $scope(e.currentTarget).attr('href');
	...
	top.on('click', '.order-listitem-button', function (e) 
	{
		...
		document.activeElement.blur();
		Grocy.FrontendHelpers.BeginUiBusy();
		const listItemId = $(e.currentTarget).attr('data-item-id');
		let done = 1;
		...
		Grocy.Api.Put('objects/shopping_list/' + listItemId, {
			done: done
		}, function () 
		{
			if (done == 1) 
		...
		const statusInfoCell = $scope('#shoppinglistitem-' + listItemId + '-status-info');
	...
	top.on('click', '#print-shopping-list-button', function (e) 
	{
		...
		const printButtons = {
			...
			printtp: {
				...
				{
					bootbox.hideAll();
					const printHeader = $scope('#print-show-header').prop('checked');
					const thermalPrintDialog = bootbox.dialog({
						title: __t('Printing'),
					...
					setTimeout(function () 
					{
						Grocy.Api.Get('print/shoppinglist/thermal?list=' + $scope('#selected-shopping-list').val() + '&printHeader=' + printHeader, function (result) 
						{
							bootbox.hideAll();
						}, function (xhr) 
						{
	...
	top.on('click', '#save-description-button', function (e) 
	{
		e.preventDefault();
		document.activeElement.blur();
		Grocy.Api.Put('objects/shopping_lists/' + $scope('#selected-shopping-list').val(), {
	...
	{
		const img = $(this);
		const barcode = img.attr('data-barcode').replace(/\D/g, '');
