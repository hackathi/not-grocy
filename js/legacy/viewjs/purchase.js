...
function purchaseView(Grocy, scope = null) 
{
	...
	$scope('#save-purchase-button').on('click', function (e) 
	{
		...
		const jsonForm = $scope('#purchase-form').serializeJSON();
		...
		Grocy.Api.Get('stock/products/' + jsonForm.product_id, function (productDetails) 
		{
			...
			{
				let amount = jsonForm.display_amount;
				...
				let price = parseFloat(jsonForm.price * $scope('#qu_id option:selected').attr('data-qu-factor')).toFixed(Grocy.UserSettings.stock_decimal_places_prices);
			...
			Grocy.Api.Post('stock/products/' + jsonForm.product_id + '/add', jsonData, function (result) 
			{
				if ($scope('#purchase-form').hasAttr('data-used-barcode')) 
				{
					Grocy.Api.Put('objects/product_barcodes/' + $scope('#purchase-form').attr('data-used-barcode'), {
						last_price: $scope('#price').val()
					}, function (result) {}, function (xhr) {});
				}
				...
				let amountMessage = parseFloat(jsonForm.amount).toLocaleString({
				...
				const successMessage = __t('Added %1$s of %2$s to stock', amountMessage + ' ' + __n(amountMessage, productDetails.quantity_unit_stock.name, productDetails.quantity_unit_stock.name_plural), productDetails.product.name) + '<br><a class="btn btn-secondary btn-sm mt-2" href="#" onclick="Grocy.UndoStockTransaction(\'' + result[0].transaction_id + '\')"><i class="fas fa-undo"></i> ' + __t('Undo') + '</a>';
				...
				else 
				{
					...
					const priceTypeUnitPrice = $scope('#price-type-unit-price');
					const priceTypeUnitPriceLabel = $scope('[for=' + priceTypeUnitPrice.attr('id') + ']');
	...
	productpicker.GetPicker().on('change', function (e) 
	{
		...
		if (productId) 
		{
			...
			Grocy.Api.Get('stock/products/' + productId, function (productDetails) 
			{
				...
				{
					$scope('#price').val(parseFloat(productDetails.last_price / $scope('#qu_id option:selected').attr('data-qu-factor')).toFixed(Grocy.UserSettings.stock_decimal_places_prices));
				}
				...
				{
					const minAmount = parseFloat(productDetails.product.tare_weight) / $scope('#qu_id option:selected').attr('data-qu-factor') + parseFloat(productDetails.stock_amount);
				...
				Grocy.FrontendHelpers.ValidateForm('purchase-form');
				const elem = productpicker.GetPicker()[0];
				...
				{
					Grocy.Api.Get('objects/product_barcodes?query[]=barcode=' + elem.getAttribute('barcode'), function (barcodeResult) 
					{
	...
	function refreshPriceHint() 
	{
		...
		{
			let amount = $scope('#display_amount').val();
			...
			let price = parseFloat($scope('#price').val() * $scope('#qu_id option:selected').attr('data-qu-factor')).toFixed(Grocy.UserSettings.stock_decimal_places_prices);
			...
			$scope('#price-hint').text(__t('means %1$s per %2$s', price.toLocaleString(undefined, {
				style: 'currency',
			...
			}), $scope('#qu_id').attr('data-destination-qu-name')));
		}
		else 
		{
			$scope('#price-hint').text('');
