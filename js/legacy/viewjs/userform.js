...
function userformView(Grocy, scope = null) 
{
	...
	function SaveUserPicture(result, jsonData) 
	{
		userfields.Save(() => 
		{
			...
			{
				Grocy.Api.UploadFile($scope('#user-picture')[0].files[0], 'userpictures', jsonData.picture_file_name, result => 
				{
	...
	$scope('#save-user-button').on('click', function (e) 
	{
		...
		const jsonData = $scope('#user-form').serializeJSON();
		...
		{
			Grocy.Api.Post('users', jsonData, result => SaveUserPicture(result, jsonData), function (xhr) 
			{
				Grocy.FrontendHelpers.EndUiBusy('user-form');
				console.error(xhr);
			});
		...
		else 
		{
			if (Grocy.DeleteUserPictureOnSave) 
			{
				...
				Grocy.Api.DeleteFile(Grocy.UserPictureFileName, 'userpictures', {}, function (result) 
				{// Nothing to do
				}, function (xhr) 
				{
					Grocy.FrontendHelpers.EndUiBusy('user-form');
			...
			Grocy.Api.Put('users/' + Grocy.EditObjectId, jsonData, result => SaveUserPicture(result, jsonData), function (xhr) 
			{
				Grocy.FrontendHelpers.EndUiBusy('user-form');
				console.error(xhr);
			});
	...
	{
		const element = $scope('#password_confirm')[0];
