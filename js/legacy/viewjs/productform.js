...
function productformView(Grocy, scope = null) 
{
	...
	Grocy.Use('numberpicker'); // preload some views.
	...
	{
		const productId = Grocy.EditObjectId || result.created_object_id;
		userfields.Save(() => 
		{
			if (Object.prototype.hasOwnProperty.call(jsonData, 'picture_file_name') && !Grocy.DeleteProductPictureOnSave) 
			{
				Grocy.Api.UploadFile($scope('#product-picture')[0].files[0], 'productpictures', jsonData.picture_file_name, result => 
				{
					...
					const returnTo = Grocy.GetUriParam('returnto');
	...
	{
		e.preventDefault();
		const jsonData = $scope('#product-form').serializeJSON();
		...
		{
			Grocy.Api.Post('objects/products', jsonData, result => saveProductPicture(result, location, jsonData), xhr => 
			{
				Grocy.FrontendHelpers.EndUiBusy('product-form');
				Grocy.FrontendHelpers.ShowGenericError('Error while saving, probably this item already exists', xhr.response);
			});
		...
		if (Grocy.DeleteProductPictureOnSave) 
		{
			...
			Grocy.Api.DeleteFile(Grocy.ProductPictureFileName, 'productpictures', {}, function (result) 
			{// Nothing to do
			}, function (xhr) 
			{
				Grocy.FrontendHelpers.EndUiBusy('product-form');
		...
		Grocy.Api.Put('objects/products/' + Grocy.EditObjectId, jsonData, result => saveProductPicture(result, location, jsonData), function (xhr) 
		{
			Grocy.FrontendHelpers.EndUiBusy('product-form');
			console.error(xhr);
		});
	...
	{
		Grocy.Api.Get('stock/products/' + Grocy.EditObjectId, function (productDetails) 
		{
	...
	{
		const quIdPurchase = $scope('#qu_id_purchase').val();
		const quIdStock = $scope('#qu_id_stock').val();
		const factor = $scope('#qu_factor_purchase_to_stock').val();
	...
	const quConversionsTable = $scope('#qu-conversions-table-products').DataTable({
		order: [[1, 'asc']],
		orderFixed: [[4, 'asc']],
		columnDefs: [{
			orderable: false,
	...
	const barcodeTable = $scope('#barcode-table').DataTable({
		order: [[1, 'asc']],
		orderFixed: [[1, 'asc']],
		columnDefs: [{
			orderable: false,
	...
	Grocy.FrontendHelpers.MakeDeleteConfirmBox('Are you sure to remove this conversion?', '.qu-conversion-delete-button', 'data-qu-conversion-id', 'data-qu-conversion-id', 'objects/quantity_unit_conversions/', (result, id, name) => 
	{
		Grocy.ProductEditFormRedirectUri = 'reload';
		$scope('#save-product-button').click();
	});
	Grocy.FrontendHelpers.MakeDeleteConfirmBox('Are you sure to remove this barcode?', '.barcode-delete-button', 'data-barcode-id', 'data-barcode-id', 'objects/product_barcodes/', (result, id, name) => 
	{
		Grocy.ProductEditFormRedirectUri = 'reload';
		$scope('#save-product-button').click();
	});
	$scope('#qu_id_stock').change(function (e) 
	{
		...
		const quIdStock = $scope('#qu_id_stock');
		const quIdPurchase = $scope('#qu_id_purchase');
	...
	{
		Grocy.Api.Get('objects/products/' + Grocy.GetUriParam('copy-of'), function (sourceProduct) 
		{
