...
function userfieldsView(Grocy, scope = null) 
{
	...
	const userfieldsTable = $scope('#userfields-table').DataTable({
		order: [[1, 'asc']],
		columnDefs: [{
			orderable: false,
			targets: 0
	...
	{
		let value = $scope('#entity-filter option:selected').text();
	...
	Grocy.FrontendHelpers.MakeDeleteConfirmBox('Are you sure to delete user field "%s"?', '.userfield-delete-button', 'data-userfield-name', 'data-userfield-id', 'objects/userfields/', '/userfields');
